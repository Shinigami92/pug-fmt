...
export class PugPrinter {
  ...
  public constructor(
    ...
    private readonly options: Required<PugFmtOptions>,
  ) {
    this.indentString =
      options.tabWidth === 'tab' ? '\t' : ' '.repeat(options.tabWidth);

    // TODO @Shinigami92 2022-10-17: Check how to provide the option of initial indentation
    // if (options.pugSingleFileComponentIndentation) {
    //   this.indentLevel++;
    // }

    this.quotes = this.options.attributeQuotes === 'single' ? "'" : '"';
    this.otherQuotes = this.quotes === "'" ? '"' : "'";

    const pugAttributeSeparator: AttributeSeparatorOption =
      resolveAttributeSeparatorOption(options.attributeSeparator);
    this.alwaysUseAttributeSeparator = pugAttributeSeparator === 'always';
    this.neverUseAttributeSeparator = pugAttributeSeparator === 'none';

    const wrapAttributesPattern: string = options.wrapAttributesPattern;
  ...
  private tag(token: TagToken): string {
    ...
    if (
      val === 'div' &&
      !this.options.explicitDiv &&
      this.nextToken &&
      ((this.nextToken.type === 'class' &&
        this.options.classLocation === 'before-attributes') ||
  ...
  private ['start-attributes'](token: StartAttributesToken): string {
    ...
    if (this.nextToken?.type === 'attribute') {
      ...
      if (this.options.classLocation === 'after-attributes') {
      ...
      if (
        !this.currentlyInPugInterpolation &&
        !this.wrapAttributes &&
        (this.currentLineLength > this.options.printWidth ||
          (this.options.wrapAttributesThreshold >= 0 &&
            numNormalAttributes > this.options.wrapAttributesThreshold))
      ) {
        this.wrapAttributes = true;
      }

      if (
        this.options.sortAttributes !== 'as-is' ||
        this.options.sortAttributesEnd.length > 0 ||
        this.options.sortAttributesBeginning.length > 0
      ) {
          ...
          this.tokens = partialSort(
            ...
            (a, b) =>
              compareAttributeToken(
                a as AttributeToken,
                b as AttributeToken,
                this.options.sortAttributes,
                this.options.sortAttributesBeginning,
                this.options.sortAttributesEnd,
              ),
          );
        }
      }
  ...
  private attribute(token: AttributeToken): void {
    formatEmptyAttribute(
      token,
      this.options.emptyAttributes,
      this.options.emptyAttributesForceQuotes,
    );
    ...
    if (typeof token.val === 'string') {
      if (isQuoted(token.val) && token.val[0] !== '`') {
        if (
          token.name === 'class' &&
          this.options.classNotation === 'literal'
        ) {
          ...
          if (normalClasses.length > 0) {
            ...
            if (this.options.classLocation === 'before-attributes') {
        ...
        } else if (token.name === 'id' && this.options.idNotation !== 'as-is') {
          // Handle id attribute
          let val: string = token.val;
          val = val.slice(1, -1);
          val = val.trim();
    ...
    if (typeof token.val === 'boolean') {
      ...
      token.name === 'class' &&
      this.options.classNotation === 'attribute'
    ) {
  ...
  private ['end-attributes'](token: EndAttributesToken): void {
    ...
    if (this.result[this.result.length - 1] === '(') {
      ...
      this.options.preserveAttributeBrackets &&
  ...
  private class(token: ClassToken): void {
    if (this.options.classNotation === 'attribute') {
      ...
      switch (this.previousToken?.type) {
        ...
        case 'indent': {
          const optionalDiv: string =
            this.options.explicitDiv ||
            this.options.classLocation === 'after-attributes'
              ? 'div'
              : '';
  ...
  private comment(commentToken: CommentToken): string {
    ...
    if (/^ prettier-ignore($|[: ])/.test(commentToken.val)) {
      ...
      result += formatPreserveCommentSpaces(
        commentToken.val,
        this.options.preserveCommentSpaces,
      );
  ...
  private text(token: TextToken): string {
    ...
    if (this.pipelessText) {
      ...
      if (this.pipelessComment) {
        val = formatPreserveCommentSpaces(
          val,
          this.options.preserveCommentSpaces,
          true,
        );
  ...
  private id(token: IdToken): void {
    ...
    switch (this.previousToken?.type) {
      ...
      case 'outdent':
      case 'indent': {
        const optionalDiv: string = this.options.explicitDiv ? 'div' : '';
